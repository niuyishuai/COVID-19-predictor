...
fprintf('--------------------------------\n');
fprintf('高斯拟合预测结果：\n');
[gauss_confirm_predictor,~]=create_Fit(date_lst,confirm_data,'gauss','新增确诊人数预测 (高斯拟合)');
fprintf('预测明日确诊人数:%d\n',round(gauss_confirm_predictor(n_days+1)));

[gauss_suspect_predictor,~]=create_Fit(date_lst,suspect_data,'gauss','新增疑似人数预测 (高斯拟合)');
fprintf('预测明日疑似人数:%d\n',round(gauss_suspect_predictor(n_days+1)));

[gauss_dead_predictor,~]=create_Fit(date_lst,dead_data,'gauss','新增死亡人数预测 (高斯拟合)');
fprintf('预测明日死亡人数:%d\n',round(gauss_dead_predictor(n_days+1)));

%% 指数拟合预测
fprintf('--------------------------------\n');
fprintf('指数拟合预测结果：\n');
[exp_confirm_predictor,~]=create_Fit(date_lst,confirm_data,'exp','新增确诊人数预测 (指数拟合)');
fprintf('预测明日确诊人数:%d\n',round(exp_confirm_predictor(n_days+1)));

[exp_suspect_predictor,~]=create_Fit(date_lst,suspect_data,'exp','新增疑似人数预测 (指数拟合)');
fprintf('预测明日疑似人数:%d\n',round(exp_suspect_predictor(n_days+1)));

[exp_dead_predictor,~]=create_Fit(date_lst,dead_data,'exp','新增死亡人数预测 (指数拟合)');
...
[smoothsplit_confirm_predictor,~]=create_Fit(date_lst,confirm_data,'smoothsplit','新增确诊人数预测 (smoothsplit)');
fprintf('预测明日确诊人数:%d\n',round(smoothsplit_confirm_predictor(n_days+1)));

[smoothsplit_suspect_predictor,~]=create_Fit(date_lst,suspect_data,'smoothsplit','新增疑似人数预测 (smoothsplit)');
fprintf('预测明日疑似人数:%d\n',round(smoothsplit_suspect_predictor(n_days+1)));

[smoothsplit_dead_predictor,~]=create_Fit(date_lst,dead_data,'smoothsplit','新增死亡人数预测 (smoothsplit)');
